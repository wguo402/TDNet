        ...
        for PCs, BBs, list_of_anno in batch: 
            ...
            for i, _ in enumerate(PCs):
                ...
                if i == 0:
                    ...
                    target_PC = kitti_utils.cropAndCenterPC(cur_PC, pre_BB, offset=dataset.offset_BB, scale=1.25, limit_area=area_extents)# (3, N2) sea
                    model_PC = kitti_utils.getModel([PCs[0], PCs[i-1]], [results_BBs[0], results_BBs[i-1]], scale=1.25)#tem
                    # step 3.1 translate to numpy
                    target_PC = target_PC[0].points 
                    model_PC = np.array(model_PC.points, dtype=np.float32)
                    # step 3.2 subsample
                    target_PC = kitti_utils.subsamplePC(target_PC, dataset.subsample_number)   
                    model_PC = kitti_utils.subsamplePC(model_PC, dataset.subsample_number//2)  
                    ...
                    if i==1:
                        model_PC1 = model_PC
                        pred_hm, pred_loc, pred_z_axis,pred_confidence = model( model_PC.unsqueeze(0).cuda(),
                                                                target_PC.unsqueeze(0).cuda(),model_PC1.unsqueeze(0).cuda())
                        target_PC_list.append(target_PC)

                    else:
                        if len(target_PC_list) <= 30:
    
                            model_PC1 = target_PC_list[-1]
                            pred_hm, pred_loc, pred_z_axis, pred_confidence = model(model_PC.unsqueeze(0).cuda(),
                                                                                    target_PC.unsqueeze(0).cuda(),
                                                                                    model_PC1.unsqueeze(0).cuda())

                            if len(target_PC_list)%30==0:
                                target_PC_list=[]
                                target_PC_list.append(target_PC)

                            if pred_confidence.sigmoid() > 0.485:
                                target_PC_list.append(target_PC)
...
global uuu
def test_model_waymo_format(easy_id_list,medium_id_list,hard_id_list,opts, model, dataset):
    ...
    for i in range(len(PCs)-1):
        ...
        if i == 0:
            ...
            # step 3. Get the point cloud
            target_PC,targe_box = kitti_utils.cropAndCenterPC(cur_PC, pre_BB, offset=offset_BB, scale=1.25, limit_area=area_extents)      # (3, N2)
            target_PC1, traget_box1, gt_box = kitti_utils.cropAndCenterPC1(cur_PC, pre_BB, this_BB,
                                                                                offset=offset_BB, scale=1.25,
                                                                                limit_area=area_extents)  # (3, N2) sea
