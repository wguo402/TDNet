...
def sample_and_group_edge(npoint, radius, nsample, xyz, points, sampling, numpoints, returnfps=False, use_knn=False):
    ...
    if sampling == "FPS":
        ...
        fps_idx = random_point_sample(xyz, numpoints) # [B, numpoints]([32, 256])

    new_xyz = index_points(xyz, fps_idx)                    # B x S x 3   ([32, 256, 3])
    
    if use_knn:
        idx = knn_point(nsample, xyz, new_xyz)# 32  ([32, 512, 3])([32, 256, 3])---->([32, 256, 32])
    else:
        idx = query_ball_point(radius, nsample, xyz, new_xyz)   # B x S x K

    grouped_xyz = index_points(xyz, idx)                        # B x S x k x 3 ([32, 512, 3])([32, 256, 32])------->([32, 256, 32, 3])
...
class PointNetSetAbstractionEdgeSA(nn.Module):
    def __init__(self, npoint, radius, nsample, mlp,mlp1, sampling, use_xyz=True, group_all=False, use_knn=False):
        ...
        # self.mlp_convs1_1=nn.Conv2d(3,32,kernel_size=(1,1),stride=(1,1))
        # self.mlp_convs1_2 = nn.Conv2d(32, 32, kernel_size=(1, 1), stride=(1, 1))
        # self.mlp_convs1_3 = nn.Conv2d(32, 32, kernel_size=(1, 1), stride=(1, 1))
        # self.mlp_bns1_1 =nn.BatchNorm2d(32, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True)
        # self.mlp_bns1_2 = nn.BatchNorm2d(32, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True)
        # self.mlp_bns1_3 = nn.BatchNorm2d(32, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True)
        ...
        # last_channel1 = mlp1[0]
        # for out_channel1 in mlp1[1:]:
        #     self.mlp_convs1.append(nn.Conv2d(last_channel1, out_channel1, 1))
        #     self.mlp_bns1.append(nn.BatchNorm2d(out_channel1))
        #     last_channel1 = out_channel1
    ...
    def forward(self, xyz, points, numpoints):
        ...
        if self.group_all:
            ...
            # new_xyz1, new_points1 = sample_and_group_all(xyz1, points1)
        else:
            ...
            # new_xyz1, new_points1 = sample_and_group_edge(self.npoint, self.radius, self.nsample, xyz1, points1,self.sampling, numpoints, False, self.use_knn)

        new_points = new_points.permute(0, 3, 1, 2)     # [B, D, numpoints, nsample]
        # new_points1 = new_points1.permute(0, 3, 1, 2)

        #new_points=new_points+new_points1
        ...
        # mlp_convs1=[self.mlp_convs1_1,self.mlp_convs1_2,self.mlp_convs1_3]
        # mlp_bns1=[self.mlp_bns1_1,self.mlp_bns1_2,self.mlp_bns1_3]
        # for i, conv1 in enumerate(self.mlp_convs1):
        #     bn1 = self.mlp_bns1[i]
        #     new_points1 = F.relu(bn1(conv1(new_points1)))

        # for j ,conv1 in enumerate(mlp_convs1):
        #     bn1 = mlp_bns1[j]
        #     new_points1 = F.relu(bn1(conv1(new_points1)))
        ...
        # for i, conv1 in enumerate(self.mlp_convs):
        #     bn1 = self.mlp_bns1[i]
        #     new_points1 = F.relu(bn1(conv1(new_points1)))


        # new_points1 = torch.max(new_points1, 3)[0]  # B x D x numpoints


        new_points = self.self_attention(new_points, new_xyz)
        # new_points1 = self.self_attention(new_points1, new_xyz1)
        # new_points=new_points+new_points1
