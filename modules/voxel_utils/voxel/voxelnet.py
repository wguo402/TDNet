...
class Conv_Middle_layers(nn.Module):
    ...
    def forward(self, x):
        out = self.conv1(x)#([32, 35, 16, 24, 38])---->([32, 64, 8, 24, 38])
        out = self.conv2(out)#([32, 64, 6, 24, 38])
        out = self.conv3(out)#([32, 64, 3, 24, 38])
        out=self.conv4(out)#[32, 128, 1, 24, 38]
        shape = out.size()
        
        out=F.max_pool3d(out,kernel_size=[shape[2], 1, 1])
        out=out.squeeze(2)#([32, 128, 24, 38])
