...
def test_tracking(opts):
    ...
    if opts.which_dataset.upper() == 'WAYMO':
        Success_run, Precision_run,Success_easy_run, Precision_easy_run,Success_medium_run, Precision_medium_run,Success_hard_run, Precision_hard_run=test_tracking_waymo_format(opts)
        return Success_run, Precision_run,Success_easy_run, Precision_easy_run,Success_medium_run, Precision_medium_run,Success_hard_run, Precision_hard_run
    else:
        # test_tracking_kitti_format(opts)
        S, P = test_tracking_kitti_format(opts)
        return S, P
...
def test_tracking_kitti_format(opts):
    ...
    test_loader, test_db, tra_iter = get_dataset(opts, partition="Test", shuffle=False)
    ...
    if (opts.n_gpus > 1) and (opts.n_gpus >= torch.cuda.device_count()):
        model = torch.nn.DataParallel(model, device_ids=list(range(opts.n_gpus)))
    ...
    interval_nuscenes = {'car': [0, 150, 1000, 2500], 'pedestrian': [0, 100, 500, 1000], 'truck': [0, 150, 1000, 2500],
                         'bicycle': [0, 100, 500, 1000]}
...
def test_tracking_waymo_format(opts):
    ...
    for tracklet_index, tracklet_info in enumerate(bench_lists[-1]):
        ...
        opts.db.tracklet_id = 'S61-KD0HmvOBLvCHvjpykg'
        opts.db.segment_name = 'segment-2094681306939952000_2972_300_2992_300_with_camera_labels'
        opts.db.frame_range =[0, 198]


        _, wod_dataset,_ = get_dataset(opts, partition="Test")
        ...
        Succ, Prec = test_model_waymo_format(easy_id_list,medium_id_list,hard_id_list,opts=opts, model=model, dataset=wod_dataset)
        ...
        print('T_F: %6d (%5d, %5d, %5d), ' % (total_frame_num, easy_frame_num, medium_frame_num, hard_frame_num),
              end='')
    ...
    print('mean Success_easy_run/Precision_easy_run %.2f/%.2f ' % (Success_easy_run.avg, Precision_easy_run.avg))
    print('mean Success_medium_run/Precision_medium_run %.2f/%.2f ' % (Success_medium_run.avg, Precision_medium_run.avg))
    print('mean Success_hard_run/Precision_hard_run %.2f/%.2f ' % (Success_hard_run.avg, Precision_hard_run.avg))

    return Success_run.avg, Precision_run.avg,Success_easy_run.avg, Precision_easy_run.avg,Success_medium_run.avg, Precision_medium_run.avg,Success_hard_run.avg, Precision_hard_run.avg
