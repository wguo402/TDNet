...
def cropAndCenterPC_withClaAndOff(PC, cut_box, gt_box, offset, scale, limit_area=None):
    ...
    # crop around box
    new_PC, class_label = cropPCwithlabel(new_PC, new_box_cut, class_label, offset=offset, scale=scale,
                                          limit_area=limit_area)

    # get offset(center & rotation)
    offcenter = np.array([new_box_gt.center[0], new_box_gt.center[1], new_box_gt.center[2],
                          new_box_gt.orientation.degrees * new_box_gt.orientation.axis[2]])
...
def subsamplePC_withClaAndOff(PC, class_label, offcenter, subsample_number, cut=False, istrain=True):
    if np.shape(PC)[1] > 2:
        ...
        if PC.shape[1] != subsample_number:
            if not istrain:
                np.random.seed(1)
            new_pts_idx = np.random.randint(low=0, high=PC.shape[1], size=subsample_number,
                                            dtype=np.int64)  # random sample
            PC = PC[:, new_pts_idx]
...
def getModel(PCs, boxes, offset=0, scale=1.0, normalize=False):
    ...
    for PC, box in zip(PCs, boxes):
        ...
        # try:
        if cropped_PC[0].points.shape[1] > 0:
            points = np.concatenate([points, cropped_PC[0].points], axis=1)
